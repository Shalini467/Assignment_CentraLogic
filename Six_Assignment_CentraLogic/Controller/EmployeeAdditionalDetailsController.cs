...
namespace EmployeeManagementSystem.Controllers
{
    ...
    public class EmployeeAdditionalDetailsController : ControllerBase
    {
        ...
        [HttpGet("{uId}")]
        public async Task<IActionResult> GetEmployeeAdditionalDetailsByUId(string uId)
        {
            
            var employee = await employeeAdditionalDetailsInterface.GetEmployeeAdditionalDetailsByUId(uId);
            var resultDTO = _mapper.Map<EmployeeAdditionDetailsDTO>(employee);
            return Ok(resultDTO);
        ...
        [HttpGet]
        public async Task<IActionResult> GetAdditionalDetailsByBasicDetailsUId(string employeeBasicDetailsUid)
        {
            var response = await employeeAdditionalDetailsInterface.GetAdditionalDetailsByBasicDetailsUId(employeeBasicDetailsUid);
            return Ok(response);
        }
